...
class ARTPrimaryLB : public Competitor {
 public:
  ...
  uint64_t Build(std::vector<KeyValue<uint64_t>>& data,
                 std::vector<EqualLookup<uint64_t>>& lookups) {
    allocated_byte_count = 0;
    data_ = &data;
    data_size_ = data.size();

    int sampling_interval = interval;
    uint64_t last_key = std::numeric_limits<uint64_t>::max();

    return util::timing([&] {
      for (uint64_t i = 0; i < data_size_ - 1; i += sampling_interval) {
        // if (interval > 1 && i % interval != 0) continue;
      ...
      const uint64_t data_key = data[data_size_ - 1].key;

      if (data_key != last_key) {
        uint8_t key[8];
        swapBytes(data_key, key);
        insert(tree_, &tree_, key, 0, data_size_ - 1, 8);
        last_key = data_key;
      }
    });
  }
...
template <int interval>
uint64_t ARTPrimaryLB<interval>::allocated_byte_count;
