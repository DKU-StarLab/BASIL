...
TEST(bitmask, tree_encode) {
  ...
  ASSERT_EQ(bitvector("10010"),
            tree_mask<8>::encode(std::bitset<8>("00001111")));
  ASSERT_EQ(bitvector("00"), tree_mask<4>::encode(std::bitset<4>("0000")));
  ASSERT_EQ(bitvector("01"), tree_mask<4>::encode(std::bitset<4>("1111")));
  ASSERT_EQ(bitvector("11000101"),
            tree_mask<8>::encode(std::bitset<8>("11110011")));
  ASSERT_EQ(bitvector("11000101"),
            tree_mask<4>::encode(std::bitset<4>("1101")));
  ASSERT_EQ(bitvector("11001001010"),
            tree_mask<4>::encode(std::bitset<4>("0101")));
  ASSERT_EQ(bitvector("11001001010"), tree_mask<32>::encode(std::bitset<32>(
...
TEST(bitmask, tree_decode) {
  ...
  ASSERT_EQ(std::bitset<8>("00001111"),
            tree_mask<8>::decode(bitvector("10010")));
  ASSERT_EQ(std::bitset<4>("0000"), tree_mask<4>::decode(bitvector("00")));
  ASSERT_EQ(std::bitset<4>("1111"), tree_mask<4>::decode(bitvector("01")));
  ASSERT_EQ(std::bitset<8>("11110011"),
            tree_mask<8>::decode(bitvector("11000101")));
  ASSERT_EQ(std::bitset<4>("1101"),
            tree_mask<4>::decode(bitvector("11000101")));
  ASSERT_EQ(std::bitset<4>("0101"),
            tree_mask<4>::decode(bitvector("11001001010")));
  ASSERT_EQ(std::bitset<32>("00000000111111110000000011111111"),
            tree_mask<32>::decode(bitvector("11001001010")));
}
...
template <u64 LEN>
static void assert_no_false_negatives(std::bitset<LEN> exact_bitmask,
                                      std::bitset<LEN> other_bitmask) {
  ASSERT_EQ(exact_bitmask, exact_bitmask & other_bitmask);
}

/// Typical blocks contain 2^17 values of type 1, 2 or 4 byte integers.
/// As we need at most cache-line granularity, bitmasks are of size 2^11, 2^12
/// or 2^13.
...
static void test_compression(const std::bitset<N> bitmask) {
  assert_no_false_negatives(
      bitmask, tree_mask<N, M>::decode(tree_mask<N, M>::compress(bitmask)));
  assert_no_false_negatives(
      bitmask, zone_mask<N, M>::decode(zone_mask<N, M>::compress(bitmask)));
};
