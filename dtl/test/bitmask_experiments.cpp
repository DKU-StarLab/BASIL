...
static u64 match_cnt_after_compression(const std::bitset<size> bitmask) {
  std::bitset<compressed_size> compressed_bitmask =
      mask_impl<size, compressed_size>::compress(bitmask);
  std::bitset<size> decompressed_bitmask =
      mask_impl<size, compressed_size>::decode(compressed_bitmask);
...
static void run() {
  ...
  std::cout
      << "actual_match_cnt|returned_match_cnt_64bit_compressed|returned_match_"
         "cnt_128bit_compressed|returned_match_cnt_256bit_compressed|returned_"
         "match_cnt_512bit_compressed|returned_match_cnt_1024bit_compressed"
      << std::endl;
  for ($u64 match_cnt = 0; match_cnt < max_match_cnt; match_cnt++) {
    for ($u64 repeat = 0; repeat < repeat_cnt; repeat++) {
      ...
      std::cout << match_cnt;
      std::cout << "|"
                << match_cnt_after_compression<size, 64, mask_impl>(bitmask);
      std::cout << "|"
                << match_cnt_after_compression<size, 128, mask_impl>(bitmask);
      std::cout << "|"
                << match_cnt_after_compression<size, 256, mask_impl>(bitmask);
      std::cout << "|"
                << match_cnt_after_compression<size, 512, mask_impl>(bitmask);
      std::cout << "|"
                << match_cnt_after_compression<size, 1024, mask_impl>(bitmask);
      std::cout << std::endl;
    }
  }
};
...
template <u64 size, u64 max_match_cnt, u64 size_limit,
          template <u64, u64> class mask_impl>
static void run_tree_mask_metrics() {
  u64 repeat_cnt = 10;
  std::cout << "bitmask_size|compressed_size_limit"
            << "|actual_match_cnt|returned_match_cnt_min|returned_match_cnt_"
               "avg|returned_match_cnt_max"
