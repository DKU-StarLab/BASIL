...
TEST(bloomfilter, sectorization_compile_time_asserts) {
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 1, true>;
    static_assert(bf_t::sector_cnt == bf_t::k, "Sector count must equal to k.");
  }
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 2, true>;
    static_assert(bf_t::sector_cnt == bf_t::k, "Sector count must equal to k.");
  }
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 3, true>;
    static_assert(bf_t::sector_cnt >= bf_t::k,
                  "Sector count must be greater or equal to k.");
  }
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 4, true>;
    static_assert(bf_t::sector_cnt == bf_t::k, "Sector count must equal to k.");
  }
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 5, true>;
    static_assert(bf_t::sector_cnt >= bf_t::k,
                  "Sector count must be greater or equal to k.");
  }
  {
    using bf_t = dtl::bloomfilter_h1<key_t, dtl::hash::knuth, word_t,
                                     dtl::mem::numa_allocator<word_t>, 6, true>;
    static_assert(bf_t::sector_cnt >= bf_t::k,
                  "Sector count must be greater or equal to k.");
  }
}
...
struct null_hash {
  ...
  static inline Ty hash(const Ty& key) { return 0; }
};

TEST(bloomfilter, k2) {
  using bf_t =
      dtl::bloomfilter_h2<key_t, dtl::hash::knuth, dtl::hash::knuth_alt, word_t,
                          dtl::mem::numa_allocator<word_t>, 6, true>;
  //  using bf_t = dtl::bloomfilter_h1<key_t,dtl::hash::knuth, word_t,
  //  dtl::mem::numa_allocator<word_t>, 4, false>;
  u32 m = 1024;
  bf_t bf(m);
  bf.print_info();
...
TEST(bloomfilter, vectorized_probe) {
  ...
  using bf_t =
      dtl::bloomfilter_h2<key_t, dtl::hash::knuth, dtl::hash::knuth_alt, word_t,
                          dtl::mem::numa_allocator<word_t>, k, sectorize>;
  using bf_vt =
      dtl::bloomfilter_h2_vec<key_t, dtl::hash::knuth, dtl::hash::knuth_alt,
                              word_t, dtl::mem::numa_allocator<word_t>, k,
                              sectorize>;
  ...
  for ($u64 i = 0; i < key_cnt; i++) {
    ...
    ASSERT_TRUE(bf.contains(key))
        << "Build failed. i = " << i << " key = " << key << std::endl;
  }
...
TEST(bloomfilter, wrapper_batch_probe) {
  ...
  for ($u32 k = 1; k <= 7; k++) {
    for ($u32 key_cnt = 1u << 5; key_cnt < 1u << 22; key_cnt <<= 1) {
      ...
      std::cout << "testing: k: " << k << ", m: " << m
                << ", key_cnt: " << key_cnt << ", m: " << m << " (" << bf.m
                << "), h: " << bf.hash_function_count() << std::endl;
      ...
      u32 match_cnt =
          bf.batch_contains(&keys[0], key_cnt, &match_pos[read_offset], 0);
      ASSERT_EQ(key_cnt, match_cnt);
      for ($u32 i = 0; i < key_cnt; i++) {
        ASSERT_EQ(i, match_pos[i + read_offset])
            << "Probe failed. i = " << i << std::endl;
      }
      bf.destruct();
    }
  }
...
TEST(bloomfilter, quality) {
  ...
  for ($u32 i = 0; i < 20; i++) {
    if (i % mod != 0) continue;
    //    std::cout << std::bitset<32>(dtl::hash::murmur1_32<u32>::hash(i)) <<
    //    std::endl;
  }
  ...
  for ($u32 k = 1; k < 8; k++) {
    ...
    auto bf = dtl::bloomfilter_runtime::construct(k, m);
    std::cout << "testing: k: " << k << ", m: " << m << ", key_cnt: " << key_cnt
              << std::endl;
    ...
    auto filter_match_cnt = std::count_if(
        keys.begin(), keys.end(), [&](u32 key) { return bf.contains(key); });

    auto fpr = 1.0 - ((actual_match_cnt * 1.0) / filter_match_cnt);
    std::cout << "false positive rate: " << fpr
              << ", approx. false positive probability: "
              << bf.false_positive_probability(actual_match_cnt)
...
TEST(bloomfilter, DISABLED_false_positive_rate_fixed_load_factor) {
  ...
  for ($f64 s = 0.0001; s < 0.01; s += 0.0001) {
    selectivities.push_back(s);
  }
  for ($f64 s = 0.1; s < 1.0; s += 0.01) {
    selectivities.push_back(s);
  }
  ...
  std::cout << "key_cnt"
            ...
            << ",match_cnt"
            << ",b"
            << ",bf_match_cnt_k1,bf_match_cnt_k2,bf_match_cnt_k3,bf_match_cnt_"
               "k4,bf_match_cnt_k5,bf_match_cnt_k6,bf_match_cnt_k7,"
            << std::endl;
  ...
  for (f64 sel : selectivities) {
    ...
    for ($u32 i = 0; i < ks.size(); i++) {
      ...
      ASSERT_EQ(sample.size(),
                std::count_if(sample.begin(), sample.end(), bf.contains));
    ...
    std::cout << key_cnt << "," << sel << "," << sample.size() << "," << b;
    std::for_each(bf_match_cnts.begin(), bf_match_cnts.end(),
...
TEST(bloomfilter, DISABLED_false_positive_rate_fixed_size_l1) {
  ...
  for (f64 sel : selectivities) {
    ...
    std::cout << key_cnt << "," << sel << "," << sample.size() << ",n/a";
    std::for_each(bf_match_cnts.begin(), bf_match_cnts.end(),
                  [](auto value) { std::cout << "," << value; });
    std::cout << std::endl;
  }
...
TEST(bloomfilter, DISABLED_determine_m) {
  ...
  //      std::cout << std::fixed << std::setprecision(12) << f << " -> m:" << m
  //      << " <- fpr:" << fpr_std(m, n, k) << " (blocked: " << fpr_blocked(m,
  //      n, k, B) << ")" << std::endl;
  //    });
  //  }
  //
  ...
  //    u64 k = 4;
  //    u64 B = 4;
  //    std::cout << std::fixed << std::setprecision(12) << fpr_std(m, n, k) <<
  //    std::endl; std::cout << std::fixed << std::setprecision(12) <<
  //    fpr_blocked(m, n, k, B) <<  std::endl;
  //  }

  {
    ...
    u64 B = 4;
    f64 f = 0.00001;
    //    std::cout << std::fixed << std::setprecision(12) << determine_m_std(f,
    //    n, k) << std::endl; std::cout << std::fixed << std::setprecision(12)
    //    << determine_m(f, n, k, B) <<  std::endl;
  }
}
