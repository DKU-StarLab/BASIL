...
TEST(knl, ht) {
  ...
  hash_t* ht_hash_values =
      mem::aligned_alloc<hash_t>(mem::cacheline_size, ht_element_cnt, 0);
  key_t* ht_keys =
      mem::aligned_alloc<key_t>(mem::cacheline_size, ht_element_cnt, 0);
  value_t* ht_values =
      mem::aligned_alloc<value_t>(mem::cacheline_size, ht_element_cnt, 0);

  // prepare the input
  u64 input_element_cnt = 1 << 10;
  key_t* input_keys =
      mem::aligned_alloc<key_t>(mem::cacheline_size, input_element_cnt);
  key_t* input_values =
      mem::aligned_alloc<value_t>(mem::cacheline_size, input_element_cnt);
  ...
  key_vec_t* ht_keys_vec =
      reinterpret_cast<key_vec_t*>(ht_keys);  // TODO: simplify
  hash_vec_t* ht_hash_values_vec =
      reinterpret_cast<hash_vec_t*>(ht_hash_values);
