...
TEST(bloom, accuracy) {
  ...
  {
    u32 worker_cnt = dtl::next_power_of_two(
        std::min(std::thread::hardware_concurrency(), 64u));
    ...
    auto data_gen_worker_fn = [&](u32 thread_id) {
      ...
      str << "# worker_id=" << thread_id << ", begin=" << range_begin
          << ", end=" << range_end << std::endl;
    ...
    for (std::size_t i = 0; i < worker_cnt; i++) {
      data.insert(std::end(data), std::begin(*worker_data[i]),
                  std::end(*worker_data[i]));
  ...
  auto generate_test_cases = [&](launch_params& p) {
    ...
    u1 continue_tests =
        current_k <= k_max && current_m_idx < ms.size() && current_n <= m_max;
    if (continue_tests) {
      p.k = current_k;
  ...
  auto worker_fn = [&](u32 thread_id) {
    ...
    while (generate_test_cases(p)) {
      ...
      result << p.k << "," << p.m << "," << p.n << ","
             << bf.hash_function_count() << "," << probe_cnt << ","
             << match_cntr << "," << (match_cntr * 1.0) / probe_cnt << ","
             << bf.pop_count() << "," << bf.load_factor() << std::endl;
      std::cout << result.str();
      bf.destruct();
    }
  };

  std::cout << "k,m,n,h,key_cnt,match_cnt,fpr,pop_count,load_factor"
            << std::endl;

  dtl::run_in_parallel(worker_fn,
                       std::min(std::thread::hardware_concurrency(), 64u));
  //  dtl::run_in_parallel(worker_fn,
  //  std::min(std::thread::hardware_concurrency(), 1u));
}
...
TEST(foo, DISABLED_bar) {
  ...
  for (size_t k = 1; k < 8; k++) {
    for (size_t m = 0; m < 5; m++) {
      ...
      auto lf_it =
          std::lower_bound(fpr_load_factor.begin(), fpr_load_factor.end(), lf);
