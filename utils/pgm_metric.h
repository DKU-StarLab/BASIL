// This source code was adapted from the following source:
// GitHub Repository: https://github.com/gre4index/GRE/blob/master/src/benchmark/pgm_metric.h
...
// Linear regression model
  ...
  public:
    ...
    explicit OptimalPiecewiseLinearModel(Y epsilon, size_t seed = 2303649288) : mse_model_builder(&mse_model), epsilon(epsilon), lower(), upper() {
      if (epsilon < 0)
...
//                  ((rank * 1.0) - intercept13 * 1.0) / slope13;
  ...
  size_t PGM_metric(KEY_TYPE *keys, int key_num, int error_bound, double *mse = nullptr) {
    ...
    auto k = keys[0];
    segment.add_point(k, 0);

    for(auto i = 1; i < key_num; i++) {
      auto next_k = keys[i];
  ...
  void PGM_metric_data_generator(uint64_t *array, int data_num, int model_num, int error_bound = 64,
    ...
    for (int i = 0; i < model_num; i++) {
      for (uint64_t j = 1; j < data_num_every_model; j++) {
      ...
      if (i == model_num - 1) {
        for (uint64_t j = 0; j < data_num - data_num_every_model * model_num; j++) {
          uint64_t key = optimal_segment.generate_key();
