...
void RadixSplineExample() {
  ...
  srs::Builder<uint64_t> rsb(min, max);
  srs::sRadixSpline<uint64_t> rs = rsb.BulkLoad(keys.data(), keys.size());

  // Search using RadixSpline.
  srs::SearchBound bound = rs.GetSearchBound(8128);
...
void MultiMapExample() {
  vector<pair<uint64_t, char>> data = {{1ull, 'a'},
                                       ...
                                       {7ull, 'c'},  // Unsorted.
                                       {42ull, 'd'}};
  srs::MultiMap<uint64_t, char> map(begin(data), end(data));
