...
class sRS {
 public:
  ...
  sRS(KeyType min_key, KeyType max_key, size_t original_num_keys,
      size_t num_radix_bits, size_t num_shift_bits, size_t interval,
      size_t max_error, std::vector<uint32_t> radix_table,
      std::vector<srs::Coord<KeyType>> spline_points)
      : min_key_(min_key),
        max_key_(max_key),
        original_num_keys_(original_num_keys),
  ...
  double GetEstimatedPosition(const KeyType key) const {
    ...
    if (key >= max_key_) return original_num_keys_ - 1;
  ...
  SearchBound GetSearchBound(const KeyType key) const {
    ...
    const size_t begin = (estimate < max_error_ + interval_ - 1)
                             ? 0
                             : estimate - max_error_ - interval_ + 1;
    const size_t end =
        (estimate + max_error_ + interval_ + 1 > original_num_keys_)
            ? original_num_keys_
            : estimate + max_error_ + interval_ + 1;
 ...
 protected:
  ...
  std::vector<srs::Coord<KeyType>> spline_points_;
