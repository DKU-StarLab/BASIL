...
class sRSBuilder {
 public:
  sRSBuilder(KeyType min_key, KeyType max_key, size_t original_num_keys_,
             size_t num_radix_bits = 18, size_t max_error = 48,
             size_t sampling_interval = 16)
      : min_key_(min_key),
        max_key_(max_key),
        num_radix_bits_(num_radix_bits),
        num_shift_bits_(GetNumShiftBits(max_key - min_key, num_radix_bits)),
        interval_(sampling_interval),
        max_error_(max_error),
        original_num_keys_(original_num_keys_),
        curr_num_keys_(0),
  ...
  sRS<KeyType, KeyValueType> BulkLoad(const std::vector<KeyValueType>& kvs) {
    assert(curr_num_keys_ == 0);

    size_t num = kvs.size();
    AddKey(kvs[0].key, 0);

    for (size_t i = interval_; i < num - 1; i += interval_) {
      AddKey(kvs[i].key, prev_position_ + interval_);
    }

    AddKey(kvs[num - 1].key, num - 1);
 ...
 protected:
  ...
  sRS<KeyType, KeyValueType> Finalize() {
    ...
    return sRS<KeyType, KeyValueType>(
        min_key_, max_key_, original_num_keys_, num_radix_bits_,
        num_shift_bits_, interval_, max_error_, std::move(radix_table_),
  ...
  // "user_error_" is not real max_error.
  ...
  size_t original_num_keys_;  // num of entire keys
  size_t curr_num_keys_;      // num of sampled keys =
                              // original_num_keys_/sampling_interval
