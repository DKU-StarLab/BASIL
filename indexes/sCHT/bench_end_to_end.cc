...
class NonOwningMultiMap {
 public:
  ...
  NonOwningMultiMap(const vector<element_type>& elements,
    ...
    scht::Builder<KeyType> chtb(min_key, max_key, num_bins, 
                            max_error/2, 2, data_.size(),
                            single_pass, ccht);
  ...
  typename vector<element_type>::const_iterator lower_bound(KeyType key) const {
    scht::SearchBound bound = cht_.GetSearchBound(key);
 ...
 private:
  ...
  scht::sCompactHistTree<KeyType> cht_;
};
