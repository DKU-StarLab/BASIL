...
WTreeDrawing::WTreeDrawing(wxWindow* parent, int id)
    : wxScrolledWindow(parent, id), wmain(NULL) {
...
void WTreeDrawing::SetWMain(WMain* wm) { wmain = wm; }
...
void WTreeDrawing::OnSize(wxSizeEvent& se) { Refresh(); }
...
wxSize WTreeDrawing::BTreeOp_Draw::draw_node(
    int offsetx, int offsety, const class BTreeType::btree_impl::node* node) {
  ...
  if (node->isleafnode()) {
    const typename btree_impl::leaf_node* leafnode =
        static_cast<const typename btree_impl::leaf_node*>(node);
    ...
    for (unsigned int slot = 0; slot < leafnode->slotuse; ++slot) {
      ...
      if (offsetx >= 0) {
        ...
        dc.DrawText(textkey, offsetx + textx + textpadding + addkeyw,
                    offsety + texty + textpadding);

        dc.DrawRectangle(offsetx + textx,
                         offsety + texty + textkeyh + 2 * textpadding - 1,
                         maxw + 2 * textpadding, textvalh + 2 * textpadding);

        dc.DrawText(
            textval, offsetx + textx + textpadding + addvalw,
            offsety + texty + textkeyh + 2 * textpadding + textpadding - 1);
      }

      textx += maxw + 2 * textpadding - 1;
      maxh = std::max(
          maxh, textkeyh + 2 * textpadding + textvalh + 2 * textpadding - 1);
    }

    return wxSize(textx, maxh);
  } else {
    const typename btree_impl::inner_node* innernode =
        static_cast<const typename btree_impl::inner_node*>(node);
    ...
    int allchildw =
        (childnum + 1) * (childmaxw / 2 + nodepadding) - 2 * nodepadding;

    if (childnum % 2 == 0) {
      allchildw += (childmaxw / 2 + nodepadding);
    }

    // debug rectangle for total size of children dc.DrawRectangle(childx,
    // childy-5, allchildw, 2);
    ...
    for (unsigned int slot = 0; slot < innernode->slotuse; ++slot) {
      ...
      if (offsetx >= 0) {
        ...
        dc.DrawRectangle(offsetx + textx, offsety + texty,
                         textkeyw + 2 * textpadding,
                         textkeyh + 2 * textpadding);
        dc.DrawText(textkey, offsetx + textx + textpadding,
                    offsety + texty + textpadding);

        // draw child
        if (innernode->level == 1) {
          // draw leaf node with border to see free slots
          dc.DrawRectangle(offsetx + childx, offsety + childy, childmaxw,
                           childmaxh);

          draw_node<BTreeType>(offsetx + childx, offsety + childy,
                               innernode->childid[slot]);
        } else {
          // draw centered inner node
          draw_node<BTreeType>(
              offsetx + childx + std::max(0, (childmaxw - childw[slot]) / 2),
              offsety + childy, innernode->childid[slot]);
        }
        ...
        splinept[2] =
            wxPoint(offsetx + childx + childmaxw / 2, offsety + childy - 20);
        splinept[3] =
            wxPoint(offsetx + childx + childmaxw / 2, offsety + childy);
    ...
    if (offsetx >= 0) {
      ...
      if (innernode->level == 1) {
        ...
        draw_node<BTreeType>(
            offsetx + childx +
                std::max(0, (childmaxw - childw[innernode->slotuse]) / 2),
            offsety + childy, innernode->childid[innernode->slotuse]);
      }
...
wxSize WTreeDrawing::BTreeOp_Draw::draw_tree(BTreeType& bt) {
  ...
  if (bt.tree.m_root) {
    ...
    if (ts.GetWidth() < w.GetSize().GetWidth()) {
      ...
      ts = draw_node<BTreeType>((w.GetSize().GetWidth() - ts.GetWidth()) / 2,
                                offsety, bt.tree.m_root);
    } else {
    ...
    if (ts != w.oldTreeSize || w.scalefactor != w.oldscalefactor) {
      ...
      w.SetScrollbars(10, 10, int(ts.GetWidth() / 10 * w.scalefactor),
