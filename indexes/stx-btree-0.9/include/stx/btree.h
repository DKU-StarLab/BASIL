...
class btree {
 ...
 public:
  ...
  template <typename Iterator>
  void sampled_bulk_load(Iterator ibegin, Iterator iend,
                         int sampling_interval) {
    BTREE_ASSERT(empty());
    m_stats.itemcount = (iend - ibegin + 1) / sampling_interval;

    if ((iend - ibegin + 1) % sampling_interval != 0) {
      m_stats.itemcount += 1;
    }

    // calculate number of leaves needed, round up.
    size_t num_items = m_stats.itemcount;
    size_t num_leaves = (num_items + leafslotmax - 1) / leafslotmax;
    ...
    for (size_t i = 0; i < num_leaves - 1; ++i) {
      ...
      // switch leaf->set_slot().
      leaf->slotuse = static_cast<int>(num_items / (num_leaves - i));
      for (size_t s = 0; s < leaf->slotuse; it += sampling_interval, ++s) {
        leaf->set_slot(s, *it);
      }
    ...
    // last leaf
    leaf_node* last_leaf = allocate_leaf();

    // copy keys or (key,value) pairs into leaf nodes, uses template
    // switch leaf->set_slot().
    last_leaf->slotuse = static_cast<int>(num_items);
    for (size_t s = 0; s < last_leaf->slotuse && it < iend;
         ++s, it += sampling_interval) {
      last_leaf->set_slot(s, *it);
    }

    if (m_tailleaf != NULL) {
      m_tailleaf->nextleaf = last_leaf;
      last_leaf->prevleaf = m_tailleaf;
    } else {
      m_headleaf = last_leaf;
    }
    m_tailleaf = last_leaf;

    num_items -= last_leaf->slotuse;
 ...
 public:
  ...
  bool erase_one(const key_type& key) {
    ...
    if (!result.has(btree_not_found)) --m_stats.itemcount;
 ...
 private:
  ...
  result_t erase_iter_descend(const iterator& iter, node* curr, node* left,
    ...
    } else  // !curr->isleafnode()
    {
      ...
      while (slot <= inner->slotuse) {
        ...
        if (slot == 0) {
          myleft = (left == NULL) ? NULL
                                  : (static_cast<inner_node*>(left))
      ...
      if (slot > inner->slotuse) return result_t(btree_not_found);
  ...
  static void shift_right_inner(inner_node* left, inner_node* right,
    ...
    // copy the parent's decision slotkey and childid to the last new key on the
    // right
    right->slotkey[shiftnum - 1] = parent->slotkey[parentslot];

    // copy the remaining last items from the left node to the first slot in the
    // right node.
    ...
    } else  // !n->isleafnode()
    {
      ...
      for (unsigned short slot = 0; slot <= inner->slotuse; ++slot) {
        ...
        if (inner->level == 2 && slot < inner->slotuse) {
          ...
          const leaf_node* leafa =
              static_cast<const leaf_node*>(parenta->childid[parenta->slotuse]);
          const leaf_node* leafb =
 ...
 public:
  ...
  bool restore(std::istream& is) {
    ...
    if (!myheader.same(fileheader)) {
      BTREE_PRINT(
          "btree::restore: file header does not match instantiation "
          "signature.");
      return false;
    }
