...
/**
 * Measures the build time for a given @p Rmi on dataset @p keys and writes
 * results to `std::cout`.
 * @tparam Key key type
 * @tparam Rmi RMI type
...
void experiment(const std::vector<key_type>& keys, const std::size_t n_models,
                ...
                const std::string layer1, const std::string layer2,
  ...
  for (std::size_t rep = 0; rep != n_reps; ++rep) {
    ...
    auto pos =
        std::lower_bound(keys.begin() + range.lo, keys.begin() + range.hi, key);
    ...
    std::cout << dataset_name << ',' << keys.size()
              << ','
              // Index
              << "ours" << ',' << layer1 << ',' << layer2 << ',' << n_models
              << ',' << bound_type << ',' << rmi.size_in_bytes()
              << ','
              // Experiment
              << rep
              << ','
 ...
 * layer 1 and layer 2 and the error bound type.
 */
struct Config {
  std::string layer1;
  std::string layer2;
...
#define ENTRIES(L1, L2, T1, T2)                                                \
  {{#L1, #L2, "labs"}, &experiment<key_type, rmi::RmiLAbs<key_type, T1, T2>>}, \
      {{#L1, #L2, "lind"},                                                     \
       &experiment<key_type, rmi::RmiLInd<key_type, T1, T2>>},                 \
      {{#L1, #L2, "gabs"},                                                     \
       &experiment<key_type, rmi::RmiGAbs<key_type, T1, T2>>},                 \
      {{#L1, #L2, "gind"},                                                     \
       &experiment<key_type, rmi::RmiGInd<key_type, T1, T2>>},                 \
...
/**
 * Triggers measurement of build times for an RMI configuration provided via
...
int main(int argc, char* argv[]) {
  ...
  if (exp_map.find(config) == exp_map.end()) {
    std::cerr << "Error: " << layer1 << ',' << layer2 << ',' << bound_type
              << " is not a valid RMI configuration." << std::endl;
    exit(EXIT_FAILURE);
  }
