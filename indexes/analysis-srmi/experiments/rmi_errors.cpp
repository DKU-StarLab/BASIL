...
/**
 * Computes several error metrics for a given @p Rmi on dataset @p keys and
...
void experiment(const std::vector<key_type>& keys, const std::size_t n_models,
  ...
  std::cout << dataset_name << ',' << n_keys
            << ','
            // RMI config
            << layer1 << ',' << layer2 << ',' << n_models
            << ','
            // Absolute error
            << mean(absolute_errors) << ',' << median(absolute_errors) << ','
            << stdev(absolute_errors) << ',' << min(absolute_errors) << ','
...
#define ENTRY(L1, L2, T1, T2)                             \
  {                                                       \
    std::make_pair(#L1, #L2),                             \
        &experiment<key_type, rmi::Rmi<key_type, T1, T2>> \
  }

static std::map<std::pair<std::string, std::string>, exp_fn_ptr> exp_map{
    ENTRY(linear_regression, linear_regression, rmi::LinearRegression,
          rmi::LinearRegression),
    ENTRY(linear_regression, linear_spline, rmi::LinearRegression,
          rmi::LinearSpline),
    ENTRY(linear_spline, linear_regression, rmi::LinearSpline,
          rmi::LinearRegression),
    ENTRY(linear_spline, linear_spline, rmi::LinearSpline, rmi::LinearSpline),
    ENTRY(cubic_spline, linear_regression, rmi::CubicSpline,
          rmi::LinearRegression),
    ENTRY(cubic_spline, linear_spline, rmi::CubicSpline, rmi::LinearSpline),
    ENTRY(radix, linear_regression, rmi::Radix<key_type>,
          rmi::LinearRegression),
...
/**
 * Triggers computation of several error metrics of an RMI configuration
