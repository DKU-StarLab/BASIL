...
/**
 * Computes statistical properties of the segments created when segmenting the
 * @p keys with @p Model and writes results to `std::cout`.
 * @tparam Key key type
...
void experiment(const std::vector<key_type>& keys, const std::size_t n_segments,
                const std::string dataset_name, const std::string model) {
  using model_type = Model;

  // Build model.
  auto m = model_type(keys.begin(), keys.end(), 0,
                      static_cast<double>(n_segments) / keys.size());
  ...
  std::cout << dataset_name << ',' << keys.size()
            ...
            << ','
            // Absolute error
            << mean(segments) << ',' << stdev(segments) << ','
            << median(segments) << ',' << min(segments) << ',' << max(segments)
            << ',' << n_empty << std::endl;
}
...
/**
 * Performs segmentation using a model type and segment count provided via
 * command line arguemnt and reports several statistical properties of the
 * resulting segments.
...
int main(int argc, char* argv[]) {
  ...
  program.add_argument("model").help(
      "model type, either linear_regression, linear_spline, cubic_spline, or "
      "radix.");
  ...
  if (exp_map.find(model) == exp_map.end()) {
    std::cerr << "Error: " << model << " is not a valid model type."
              << std::endl;
    exit(EXIT_FAILURE);
  }
