...
/**
 * Computes several metrics on the error interval sizes for a given @p Rmi on
 * dataset @p keys and writes results to `std::cout`.
 * @tparam Key key type
 * @tparam Rmi RMI type
...
void experiment(const std::vector<key_type>& keys, const std::size_t n_models,
  ...
  std::cout << dataset_name << ',' << n_keys
            ...
            << ','
            // Interval sizes
            << mean(interval_sizes) << ',' << median(interval_sizes) << ','
            << stdev(interval_sizes) << ',' << min(interval_sizes) << ','
...
/**
 * Triggers computation of several metrics on the error interval sizes of an RMI
