...
/**
 * Builds recursive model indexes of different size on @p keys and performs @p
 * n_reps of lookups on @p samples. Writes results including build time,
 * evaluation time, and lookup time to `std::cout`.
...
void benchmark_rmi(const std::vector<key_type>& keys,
                   ...
                   const std::size_t n_reps, const std::string dataset_name) {
  // Set hyperparameters.
...
#define RUN(RMI_TYPE, SEARCH_FN, N_MODELS)                                     \
  {                                                                            \
    ...
    for (std::size_t rep = 0; rep != n_reps; ++rep) {                          \
      ...
      /* Report results. */                                                    \
      /* Dataset */                                                            \
      std::cout << dataset_name << ',' << keys.size() << ',' /* Index */       \
                << "RMI-ours" << ',' << "\"" << #RMI_TYPE << ','               \
                << "layer2_size=" << N_MODELS << "\"" << ','                   \
                << rmi.size_in_bytes() << ','          /* Experiment */        \
                << rep << ',' << samples.size() << ',' /* Results */           \
                << build_time << ',' << eval_time << ',' << lookup_time        \
                << ',' /* Checksums */                                         \
                << eval_accu << ',' << lookup_accu << std::endl;               \
    } /* reps */                                                               \
  }
...
/**
 * Builds ALEX of different size on @p keys and performs @p n_reps of lookups on
 * @p samples. Writes results including build time, evaluation time, and lookup
 * time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
 * @param n_reps number of repetitions
...
/**
 * Builds PGM-indexes of different size on @p keys and performs @p n_reps of
 * lookups on @p samples. Writes results including build time, evaluation time,
 * and lookup time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
 * @param n_reps number of repetitions
 * @param dataset_name name of the dataset
  ...
  {                                                                          \
    ...
    for (std::size_t rep = 0; rep != n_reps; ++rep) {                        \
      ...
      for (std::size_t i = 0; i != samples.size(); ++i) {                    \
        auto key = samples.at(i);                                            \
        auto range = pgm.search(key);                                        \
        auto pos = std::lower_bound(keys.begin() + range.lo,                 \
                                    keys.begin() + range.hi, key);           \
        lookup_accu += std::distance(keys.begin(), pos);                     \
      }                                                                      \
      stop = steady_clock::now();                                            \
      auto lookup_time = duration_cast<nanoseconds>(stop - start).count();   \
      ...
      /* Dataset */                                                          \
      std::cout << dataset_name << ',' << keys.size() << ',' /* Index */     \
                << "PGM-index" << ',' << "\"epsilon=" << epsilon             \
                << ",epsilon_recursive=" << epsilon_recursive << "\"" << ',' \
                << pgm.size_in_bytes() << ','          /* Experiment */      \
                << rep << ',' << samples.size() << ',' /* Results */         \
                << build_time << ',' << eval_time << ',' << lookup_time      \
                << ',' /* Checksums */                                       \
                << eval_accu << ',' << lookup_accu << std::endl;             \
...
/**
 * Builds RadixSplines of different size on @p keys and performs @p n_reps of
 * lookups on @p samples. Writes results including build time, evaluation time,
 * and lookup time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
  ...
  for (auto num_radix_bits : radix_bits) {
    for (auto max_error : max_errors) {
      ...
      for (std::size_t rep = 0; rep != n_reps; ++rep) {
        // Build time.
        auto start = steady_clock::now();
        rs::Builder<key_type> rsb(keys.front(), keys.back(), num_radix_bits,
                                  max_error);
        ...
        for (std::size_t i = 0; i != samples.size(); ++i) {
          auto key = samples.at(i);
          auto range = rs.GetSearchBound(key);
          auto pos = std::lower_bound(keys.begin() + range.begin,
                                      keys.begin() + range.end, key);
          lookup_accu += std::distance(keys.begin(), pos);
        }
        stop = steady_clock::now();
        auto lookup_time = duration_cast<nanoseconds>(stop - start).count();
        ...
        std::cout << dataset_name << ',' << keys.size()
                  << ','
                  // Index
                  << "RadixSpline" << ',' << "\"max_error=" << max_error
...
/**
 * Builds Compact Hist-Trees of different size on @p keys and performs @p n_reps
 * of lookups on @p samples. Writes results including build time, evaluation
 * time, and lookup time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
 * @param n_reps number of repetitions
 * @param dataset_name name of the dataset
...
void benchmark_cht(const std::vector<key_type>& keys,
  ...
  std::vector<std::pair<std::size_t, std::size_t>> configs = {
      {16, 512},   {32, 512},   {64, 16},   {64, 32},   {64, 64},   {64, 128},
      {64, 256},   {64, 512},   {128, 16},  {128, 32},  {128, 64},  {128, 128},
      {128, 256},  {128, 512},  {256, 16},  {256, 32},  {256, 64},  {256, 128},
      {256, 256},  {256, 512},  {512, 16},  {512, 32},  {512, 64},  {512, 128},
      {512, 256},  {512, 512},  {1024, 16}, {1024, 32}, {1024, 64}, {1024, 128},
      {1024, 256}, {1024, 512},
  };
  ...
  for (auto config : configs) {
    ...
    for (std::size_t rep = 0; rep != n_reps; ++rep) {
      // Build time.
      auto start = steady_clock::now();
      cht::Builder<key_type> chtb(keys.front(), keys.back(), num_bins,
                                  max_error, false, false);
      ...
      std::cout << dataset_name << ',' << keys.size()
                << ','
                // Index
                << "Compact Hist-Tree" << ',' << "\"num_bins=" << num_bins
                << ",max_error=" << max_error << "\"" << ',' << cht.GetSize()
                << ','
                // Experiment
                << rep << ',' << samples.size()
                << ','
...
/**
 * Builds Adaptive Radix Trees of different size on @p keys and performs @p
 * n_reps of lookups on @p samples. Writes results including build time,
 * evaluation time, and lookup time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
  ...
  for (std::size_t k = min_sparcity; k <= max_sparcity; k++) {
    ...
    for (std::size_t rep = 0; rep != n_reps; ++rep) {
      ...
      for (std::size_t i = 0; i != samples.size(); ++i) {
        auto key = samples.at(i);
        auto range = art.search(key);
        auto pos = std::lower_bound(keys.begin() + range.first,
                                    keys.begin() + range.second, key);
        lookup_accu += std::distance(keys.begin(), pos);
      }
      stop = steady_clock::now();
      auto lookup_time = duration_cast<nanoseconds>(stop - start).count();
...
/**
 * Builds B-trees of different size on @p keys and performs @p n_reps of lookups
 * on @p samples. Writes results including build time, evaluation time, and
 * lookup time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
 * @param n_reps number of repetitions
 * @param dataset_name name of the dataset
...
void benchmark_tlx(const std::vector<key_type>& keys,
  ...
  for (std::size_t k = min_sparcity; k <= max_sparcity; k++) {
    ...
    for (std::size_t rep = 0; rep != n_reps; ++rep) {
      ...
      auto inner_node_size =
          inner_slots * sizeof(key_type) +
          (inner_slots + 1) * sizeof(void*);  // keys and pointers

      auto leaf_slots = stats.leaf_slots;
      auto n_leaves = stats.leaves;
      auto leaf_size = 2 * sizeof(void*) +
                       leaf_slots * (sizeof(key_type) +
                                     sizeof(uint64_t));  // prev/next + data

      double size_in_bytes =
          inner_node_size * n_inner_nodes + leaf_size * n_leaves;
...
/**
 * Loads pre-built recursive model indexes and performs @p n_reps of lookups on
 * @p samples.  Writes results including build time, evaluation time, and lookup
 * time to `std::cout`.
 * @param keys on which the index is built
 * @param samples used for measuring the lookup time
...
#define RUN(NAMESPACE)                                                        \
  ...
  for (std::size_t rep = 0; rep != n_reps; ++rep) {                           \
    ...
    /* Dataset */                                                             \
    std::cout << dataset_name << ',' << keys.size() << ',' /* Index */        \
              << "RMI-ref" << ',' << #NAMESPACE << ',' << size_in_bytes       \
              << ','                                 /* Experiment */         \
              << rep << ',' << samples.size() << ',' /* Results */            \
              << build_time << ',' << eval_time << ',' << lookup_time         \
              << ',' /* Checksums */                                          \
  ...
  if (dataset_name == "books_200M_uint64") {
    ...
    std::cerr << "Reference implementation RMI not pre-trained for given "
                 "dataset. Skipping."
              << std::endl;
    return;
  }
...
/**
 * Performs an index comparison in terms of build time, evaluation time, and
 * lookup time.
...
int main(int argc, char* argv[]) {
  ...
  program.add_argument("--ref")
      .help(
          "run benchmark on reference implementation of Recursive Model Index")
      .default_value(false)
      .implicit_value(true);
  ...
  // Run benchmarks.
  if (program["--rmi"] == true)
    benchmark_rmi(keys, samples, n_reps, dataset_name);
  if (program["--alex"] == true)
    benchmark_alex(keys, samples, n_reps, dataset_name);
  if (program["--pgm"] == true)
    benchmark_pgm(keys, samples, n_reps, dataset_name);
  if (program["--rs"] == true)
    benchmark_rs(keys, samples, n_reps, dataset_name);
  if (program["--cht"] == true)
    benchmark_cht(keys, samples, n_reps, dataset_name);
  if (program["--art"] == true)
    benchmark_art(keys, samples, n_reps, dataset_name);
  if (program["--tlx"] == true)
    benchmark_tlx(keys, samples, n_reps, dataset_name);
  if (program["--ref"] == true)
    benchmark_ref(keys, samples, n_reps, dataset_name);
  if (program["--bin"] == true)
    benchmark_bin(keys, samples, n_reps, dataset_name);
