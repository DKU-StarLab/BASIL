...
/**
 * Measures lookup times of @p samples on a given @p Rmi and writes results to
 * `std::cout`.
 * @tparam Key key type
 * @tparam Rmi RMI type
...
void experiment(const std::vector<key_type>& keys, const std::size_t n_models,
                const std::vector<key_type>& samples, const std::size_t n_reps,
                const std::string dataset_name, const std::string layer1,
                const std::string layer2, const std::string bounds,
                const std::string search, const std::size_t budget,
  ...
  for (std::size_t rep = 0; rep != n_reps; ++rep) {
    ...
    for (std::size_t i = 0; i != samples.size(); ++i) {
      ...
      auto pos = search_fn(keys.begin() + range.lo, keys.begin() + range.hi,
                           keys.begin() + range.pos, key);
    ...
    // Report results.
    // Dataset
    std::cout << dataset_name << ',' << keys.size()
              << ','
              // Index
              << layer1 << ',' << layer2 << ',' << n_models << ',' << bounds
              << ',' << search << ',' << rmi.size_in_bytes()
              << ','
              // Experiment
              << rep << ',' << samples.size() << ',' << budget << ','
              << is_guideline
              << ','
              // Results
              << lookup_time
 ...
 */
typedef void (*exp_fn_ptr)(const std::vector<key_type>&, const std::size_t,
                           const std::vector<key_type>&, const std::size_t,
                           const std::string, const std::string,
                           const std::string, const std::string,
                           const std::string, const std::size_t, const bool);

/**
 * RMI configuration that holds the string representation of model types of
 * layer 1 and layer 2, error bound type, and search algorithm.
 */
struct Config {
  std::string layer1;
  std::string layer2;
...
#define ENTRIES(L1, L2, LT1, LT2)                                              \
  {{#L1, #L2, "none", "binary"},                                               \
   &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>, BinarySearch>},         \
      {{#L1, #L2, "labs", "binary"},                                           \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>, BinarySearch>}, \
      {{#L1, #L2, "lind", "binary"},                                           \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>, BinarySearch>}, \
      {{#L1, #L2, "gabs", "binary"},                                           \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>, BinarySearch>}, \
      {{#L1, #L2, "gind", "binary"},                                           \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>, BinarySearch>}, \
      {{#L1, #L2, "none", "model_biased_binary"},                              \
       &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>,                     \
                   ModelBiasedBinarySearch>},                                  \
      {{#L1, #L2, "labs", "model_biased_binary"},                              \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedBinarySearch>},                                  \
      {{#L1, #L2, "lind", "model_biased_binary"},                              \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>,                 \
                   ModelBiasedBinarySearch>},                                  \
      {{#L1, #L2, "gabs", "model_biased_binary"},                              \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedBinarySearch>},                                  \
      {{#L1, #L2, "gind", "model_biased_binary"},                              \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>,                 \
                   ModelBiasedBinarySearch>},                                  \
      {{#L1, #L2, "none", "linear"},                                           \
       &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>, LinearSearch>},     \
      {{#L1, #L2, "labs", "linear"},                                           \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>, LinearSearch>}, \
      {{#L1, #L2, "lind", "linear"},                                           \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>, LinearSearch>}, \
      {{#L1, #L2, "gabs", "linear"},                                           \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>, LinearSearch>}, \
      {{#L1, #L2, "gind", "linear"},                                           \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>, LinearSearch>}, \
      {{#L1, #L2, "none", "model_biased_linear"},                              \
       &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>,                     \
                   ModelBiasedLinearSearch>},                                  \
      {{#L1, #L2, "labs", "model_biased_linear"},                              \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedLinearSearch>},                                  \
      {{#L1, #L2, "lind", "model_biased_linear"},                              \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>,                 \
                   ModelBiasedLinearSearch>},                                  \
      {{#L1, #L2, "gabs", "model_biased_linear"},                              \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedLinearSearch>},                                  \
      {{#L1, #L2, "gind", "model_biased_linear"},                              \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>,                 \
                   ModelBiasedLinearSearch>},                                  \
      {{#L1, #L2, "none", "exponential"},                                      \
       &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>,                     \
                   ExponentialSearch>},                                        \
      {{#L1, #L2, "labs", "exponential"},                                      \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>,                 \
                   ExponentialSearch>},                                        \
      {{#L1, #L2, "lind", "exponential"},                                      \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>,                 \
                   ExponentialSearch>},                                        \
      {{#L1, #L2, "gabs", "exponential"},                                      \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>,                 \
                   ExponentialSearch>},                                        \
      {{#L1, #L2, "gind", "exponential"},                                      \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>,                 \
                   ExponentialSearch>},                                        \
      {{#L1, #L2, "none", "model_biased_exponential"},                         \
       &experiment<key_type, rmi::Rmi<key_type, LT1, LT2>,                     \
                   ModelBiasedExponentialSearch>},                             \
      {{#L1, #L2, "labs", "model_biased_exponential"},                         \
       &experiment<key_type, rmi::RmiLAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedExponentialSearch>},                             \
      {{#L1, #L2, "lind", "model_biased_exponential"},                         \
       &experiment<key_type, rmi::RmiLInd<key_type, LT1, LT2>,                 \
                   ModelBiasedExponentialSearch>},                             \
      {{#L1, #L2, "gabs", "model_biased_exponential"},                         \
       &experiment<key_type, rmi::RmiGAbs<key_type, LT1, LT2>,                 \
                   ModelBiasedExponentialSearch>},                             \
      {{#L1, #L2, "gind", "model_biased_exponential"},                         \
       &experiment<key_type, rmi::RmiGInd<key_type, LT1, LT2>,                 \
                   ModelBiasedExponentialSearch>},

static std::map<Config, exp_fn_ptr, ConfigCompare> exp_map{
    ENTRIES(linear_regression, linear_regression, rmi::LinearRegression,
            rmi::LinearRegression) ENTRIES(linear_regression, linear_spline,
                                           rmi::LinearRegression,
                                           rmi::LinearSpline)
        ENTRIES(linear_spline, linear_regression, rmi::LinearSpline,
                rmi::LinearRegression) ENTRIES(linear_spline, linear_spline,
                                               rmi::LinearSpline,
                                               rmi::LinearSpline)
            ENTRIES(cubic_spline, linear_regression, rmi::CubicSpline,
                    rmi::LinearRegression) ENTRIES(cubic_spline, linear_spline,
                                                   rmi::CubicSpline,
                                                   rmi::LinearSpline)
                ENTRIES(radix, linear_regression, rmi::Radix<key_type>,
                        rmi::LinearRegression)
                    ENTRIES(
                        radix, linear_spline, rmi::Radix<key_type>,
                        rmi::LinearSpline)};  ///< Map that assigns an
                                              ///< experiment function pointer
                                              ///< to RMI configurations.
#undef ENTRIES

/*
 * Computes the recommended RMI configuration by following a simple guideline
...
void evaluate_guideline(const std::vector<key_type>& keys,
  ...
  auto n_models = (budget - 2 * sizeof(double) - 2 * sizeof(std::size_t)) /
                  (2 * sizeof(double));

  // Train RMI.
  rmi::Rmi<key_type, rmi::LinearSpline, rmi::LinearRegression> rmi(keys,
                                                                   n_models);
  ...
  if (mean_log2e < threshold) {
    ...
    (*exp_fn)(keys, n_models, samples, n_reps, dataset_name, l1, l2, bounds,
              search, budget, true);
  } else {
    auto bounds = "labs";
    auto search = "binary";
...
/**
 * Tests RMI configurations for a given size budget and compares them against
 * the performance chosen by the guideline in termns of lookup time.
...
int main(int argc, char* argv[]) {
  ...
  std::vector<std::string> l1_models = {"linear_spline", "cubic_spline",
                                        "linear_regression", "radix"};
  std::vector<std::string> l2_models = {
      "linear_regression"};  // We know that lr is always better than ls from
  ...
  for (auto l1 : l1_models) {
    for (auto l2 : l2_models) {
      for (auto corr : err_corrs) {
        ...
        auto n_models = (budget - model_size[l1] - 2 * sizeof(std::size_t)) /
                        (model_size[l2] + bounds_size[bounds]);
