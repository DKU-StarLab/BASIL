...
class PGMWrapper : public pgm::PGMIndex<K, 1, EPSILON_RECURSIVE> {
 ...
 public:
  PGMWrapper(const K* a, size_t n, size_t epsilon) : epsilon(epsilon) {
    ...
    this->build(a, a + n, epsilon, EPSILON_RECURSIVE, this->segments,
                this->levels_offsets);
  }
...
#define PGM_INDEX_DEFINE(type)                                                 \
  ...
  void pgm_index_##type##_destroy(PGM_PTR(pgm_index, type) pgm) {              \
    delete pgm;                                                                \
                                                                               ...
                                                                               \
  approx_pos_t pgm_index_##type##_search(PGM_PTR(pgm_index, type) pgm,         \
                                         PGM_T(type) q) {                      \
    return pgm->search(q);                                                     \
  }                                                                            \
                                                                               \
...
#define DYNAMIC_PGM_INDEX_DEFINE(type)                                         \
  struct dynamic_pgm_index_##type##_                                           \
      : public pgm::DynamicPGMIndex<PGM_T(type), PGM_T(type)> {                \
    using pgm::DynamicPGMIndex<PGM_T(type), PGM_T(type)>::DynamicPGMIndex;     \
  };                                                                           \
                                                                               \
  PGM_PTR(dynamic_pgm_index, type)                                             \
  dynamic_pgm_index_##type##_create(const pair_##type##_t* a, size_t n) {      \
    try {                                                                      \
  ...
  PGM_PTR(dynamic_pgm_index, type) dynamic_pgm_index_##type##_create_empty() { \
    return new dynamic_pgm_index_##type##_();                                  \
                                                                               ...
                                                                               \
  void dynamic_pgm_index_##type##_destroy(PGM_PTR(dynamic_pgm_index, type)     \
                                              pgm) {                           \
    delete pgm;                                                                \
  }                                                                            \
                                                                               \
  size_t dynamic_pgm_index_##type##_size(PGM_PTR(dynamic_pgm_index, type)      \
                                             pgm) {                            \
    return pgm->size();                                                        \
  }                                                                            \
                                                                               \
  size_t dynamic_pgm_index_##type##_size_in_bytes(                             \
      PGM_PTR(dynamic_pgm_index, type) pgm) {                                  \
    return pgm->size_in_bytes();                                               \
  }                                                                            \
                                                                               \
  size_t dynamic_pgm_index_##type##_index_size_in_bytes(                       \
      PGM_PTR(dynamic_pgm_index, type) pgm) {                                  \
    return pgm->index_size_in_bytes();                                         \
  }                                                                            \
                                                                               \
  void dynamic_pgm_index_##type##_insert_or_assign(                            \
      PGM_PTR(dynamic_pgm_index, type) pgm, PGM_T(type) key,                   \
      PGM_T(type) value) {                                                     \
    pgm->insert_or_assign(key, value);                                         \
  }                                                                            \
                                                                               \
  void dynamic_pgm_index_##type##_erase(PGM_PTR(dynamic_pgm_index, type) pgm,  \
                                        PGM_T(type) key) {                     \
    pgm->erase(key);                                                           \
  }                                                                            \
                                                                               \
  bool dynamic_pgm_index_##type##_find(PGM_PTR(dynamic_pgm_index, type) pgm,   \
                                       PGM_T(type) key, PGM_T(type) * value) { \
  ...
  bool dynamic_pgm_index_##type##_iterator_next(                               \
    ...
    *value = (*iter)->second;                                                  \
    ...
    return true;                                                               \
                                                                               ...
                                                                               \
  void dynamic_pgm_index_##type##_iterator_destroy(                            \
      dynamic_pgm_index_##type##_iterator_t it) {                              \
