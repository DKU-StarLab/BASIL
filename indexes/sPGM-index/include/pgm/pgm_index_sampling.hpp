...
#include "piecewise_linear_model_sampling.hpp"
#include <algorithm>
#include <cstddef>
#include <cstdint>
#include <iterator>
...
#define SPGM_SUB_EPS(x, epsilon, interval) (((x) <= (epsilon) + (interval) - (1)) ? 0 : ((x) - (epsilon) - (interval) + 1))
#define SPGM_ADD_EPS(x, epsilon, interval, size) (((x) + (epsilon) + (interval) + (1) >= (size)) ? (size) : ((x) + (epsilon) + (interval) + (1)))
...
protected:
    ...
    static void build(RandomIt first, RandomIt last,
                      ...
                      size_t interval, size_t interval_recursive,
        ...
        auto ignore_last = ((*std::prev(last)).key) == std::numeric_limits<K>::max(); // max() is the sentinel value
        auto last_n = n - ignore_last;
        last -= ignore_last;

        auto build_sampled_level = [&](auto epsilon, auto interval, auto in_fun, auto out_fun) {
            size_t n_segments = 0;
            
            if (interval == 1){
                n_segments = internal::make_segmentation(last_n, epsilon, in_fun, out_fun);
            } else {
                n_segments = internal::make_segmentation_sampled(last_n, epsilon, interval, in_fun, out_fun);
            }

            if (last_n > 1 && segments.back().slope == 0) {
                // Here we need to ensure that keys > *(last-1) are approximated to a position == prev_level_size
                segments.emplace_back(((*std::prev(last)).key) + 1, 0, last_n);
                ++n_segments;
            }
        ...
        auto in_fun = [&](auto i) {
            ...
            auto flag = ((i > 0) && (i + 1u < n) && (x == (first[i - 1].key)) && (x != (first[i + 1].key)) && (x + 1 != (first[i + 1].key)));
            return std::pair<K, size_t>(x + flag, i);
        };
        auto out_fun = [&](auto cs) { segments.emplace_back(cs); };
  
        last_n = build_sampled_level(epsilon, interval, in_fun, out_fun);
        ...
        while (epsilon_recursive && last_n > 1) {
            ...
            last_n = build_sampled_level(epsilon_recursive, interval_recursive, in_fun_rec, out_fun);
            levels_offsets.push_back(levels_offsets.back() + last_n + 1);
        }
    }
    ...
    auto segment_for_key(const K &key) const {
        if constexpr ((EpsilonRecursive + IntervalRecursive - 1) == 0) {
        ...
        for (auto l = int(height()) - 2; l >= 0; --l) {
            ...
            auto lo = level_begin + SPGM_SUB_EPS(pos, EpsilonRecursive + 1, IntervalRecursive);

            static constexpr size_t linear_search_threshold = 8 * 64 / sizeof(Segment);
            if constexpr (EpsilonRecursive + IntervalRecursive - 1 <= linear_search_threshold) {
                for (; std::next(lo)->key <= key; ++lo)
                    continue;
                it = lo;
            } else {
                auto level_size = levels_offsets[l + 1] - levels_offsets[l] - 1;
                auto hi = level_begin + SPGM_ADD_EPS(pos, EpsilonRecursive, IntervalRecursive, level_size);
...
public:
     ...
     */
    explicit sPGMIndex(const std::vector<KV> &data) : sPGMIndex(data.begin(), data.end()) {}
     ...
     */
    template<typename RandomIt>
    sPGMIndex(RandomIt first, RandomIt last)
        : n(std::distance(first, last)),
          first_key(n ? ((*first).key) : K(0)),
          segments(),
          levels_offsets() {
          build(first, last, Epsilon, EpsilonRecursive, Interval, IntervalRecursive,
             segments, levels_offsets);
        }
    ...
    ApproxPos search(const K &key) const {
        ...
        auto lo = SPGM_SUB_EPS(pos, Epsilon, Interval);
        auto hi = SPGM_ADD_EPS(pos, Epsilon, Interval, n);   
        return {pos, lo, hi};
    }
...
template<typename K, typename KV, size_t Epsilon, size_t Interval, size_t EpsilonRecursive, size_t IntervalRecursive, typename Floating>
struct sPGMIndex<K, KV, Epsilon, Interval, EpsilonRecursive, IntervalRecursive, Floating>::Segment {
