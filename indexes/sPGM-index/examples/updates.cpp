/*
 * This example shows how to use pgm::DynamicPGMIndex, a std::map-like container
 * supporting inserts and deletes. Compile with: g++ updates.cpp -std=c++17
 * -I../include -o updates Run with:
 *   ./updates
 */
...
int main() {
  ...
  pgm::DynamicPGMIndex<uint32_t, uint32_t> dynamic_pgm(data.begin(),
                                                       data.end());
  ...
  // Query the container
  std::cout << "Container size (data + index) = " << dynamic_pgm.size_in_bytes()
            << " bytes" << std::endl;
  std::cout << "find(4) = "
            << (dynamic_pgm.find(4) == dynamic_pgm.end() ? "not found"
                                                         : "found")
            << std::endl;
  ...
  for (auto [k, v] : result) std::cout << "(" << k << "," << v << "), ";
