/*
 * This example shows how to use pgm::MappedPGMIndex, a container that stores
 * its data on disk and uses the PGM-index to support efficient query
 * operations. Compile with: g++ mapped.cpp -std=c++17 -I../include -o mapped
 * Run with:
 *   ./mapped
 */
...
int main() {
...

  {
    ...
    std::cout << "Created indexed file at " << tmp_filename << std::endl
              ...
              << "- file size " << pgm.file_size_in_bytes() << " bytes"
              << std::endl
...

  {
    ...
    std::cout << "File re-opened" << std::endl
              ...
              << "contains(1) = " << (pgm.contains(1) ? "true" : "false")
              << std::endl
