/*
 * This example shows how to use the PGM-index to implement classical query
 * operations on a sorted multiset. Compile with: g++ multiset.cpp -std=c++17
 * -I../include -o multiset Run with:
 *   ./multiset
 */
...
class PGMMultiset {
 ...
 public:
  explicit PGMMultiset(const std::vector<K>& data)
      : data(data), pgm(data.begin(), data.end()) {}

  bool contains(const K x) const {
    auto range = pgm.search(x);
    return std::binary_search(data.begin() + range.lo, data.begin() + range.hi,
                              x);
  }

  auto lower_bound(const K x) const {
    auto range = pgm.search(x);
    return std::lower_bound(data.begin() + range.lo, data.begin() + range.hi,
                            x);
  }
  ...
  auto upper_bound(const K x) const {
    auto range = pgm.search(x);
    auto it =
        std::upper_bound(data.begin() + range.lo, data.begin() + range.hi, x);
    auto step = 1ull;
    while (it + step < end() &&
           *(it + step) == x)  // exponential search to skip duplicates
      step *= 2;
...
int main() {
  ...
  std::cout << "contains(1) = " << (multiset.contains(1) ? "true" : "false")
            << std::endl;
